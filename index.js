...
async function sessionDownload(sessionId, number, retries = 3) {
    ...
    // For local sessions, just check if file exists
...
async function uploadCredsToMega(credsPath) {
    try {
        // Return local path instead of MEGA upload
        const fileName = path.basename(credsPath);
        return `local://${fileName}`;
    } catch (error) {
        // Return local path on error
        const fileName = path.basename(credsPath);
        return `local://${fileName}`;
    }
}
        ...
        socket.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'close') {
                ...
                try {
                    ...
                    const sid = 'SESSION-LOCAL-' + Date.now();
